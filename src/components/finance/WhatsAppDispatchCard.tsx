...
import { MessageSquare, Check, Sparkles, ShieldCheck, Smartphone, Send, Loader2, ExternalLink, Bell } from "lucide-react";
...
export function WhatsAppDispatchCard({
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("useaimly_whatsapp_settings");
      if (saved) {
        ...
        if (typeof parsed.notifyGoalPace === "boolean") setNotifyGoalPace(parsed.notifyGoalPace);
        if (typeof parsed.notifyExpenses === "boolean") setNotifyExpenses(parsed.notifyExpenses);
        if (typeof parsed.notifyAICoaching === "boolean") setNotifyAICoaching(parsed.notifyAICoaching);
      }
    } catch (e) {
      console.warn("Failed to load WhatsApp settings from localStorage", e);
    }
  }, []);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      localStorage.setItem("useaimly_whatsapp_settings", JSON.stringify(settings));
      
      // Save to Supabase user profile if authenticated
      ...
      if (session?.session?.user) {
        await supabase
          ...
          .update({
            ...
            notify_via_whatsapp: isEnabled,
          })
          .eq("id", session.session.user.id);
      }
    } catch (err) {
      console.warn("Local storage or Supabase save warning:", err);
    }
    ...
    try {
      const res = await fetch("/api/notifications/whatsapp", {
        ...
        body: JSON.stringify({
          ...
          goalTitle: destinationTitle,
          daysRemaining: delayInDays > 0 ? delayInDays : 30,
          targetDate,
          projectedDate,
          monthlyCapacity: monthlyGoalCapacity,
          currency,
          aiTip: notifyAICoaching
            ? "You are maintaining 100% pace. Your liquid reserves protect fixed living costs."
            : "Keep monitoring monthly expenses to ensure target date stability.",
          triggers: {
            goalPace: notifyGoalPace,
            expenses: notifyExpenses,
            aiCoaching: notifyAICoaching,
          },
        }),
      });
  ...
  return (
    <div className="rounded-3xl border-2 border-emerald-500/30 bg-card p-6 sm:p-8 space-y-6 shadow-xl relative overflow-hidden">
      ...
      {/* Grid: Left Settings Form | Right Live Preview */}
        ...
        <form onSubmit={handleSave} className="lg:col-span-7 space-y-4">
          ...
          <div className="pt-3 flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              className="rounded-xl border border-border/80 bg-secondary/50 text-foreground font-semibold text-xs px-4 py-2.5 hover:bg-secondary transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
            >
              {isSendingTest ? (
                <>
            ...
            {dispatchResult?.whatsappUrl && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="rounded-xl border border-emerald-500/40 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 font-bold text-xs px-4 py-2.5 hover:bg-emerald-500/20 transition-all flex items-center gap-1.5 cursor-pointer"
              >
          ...
          {dispatchResult?.message && (
            <p className="text-[11px] font-mono text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 p-2.5 rounded-xl">
              {dispatchResult.message}
            </p>
          )}
        </form>

        {/* Right Phone WhatsApp Screen Live Simulation */}
        <div className="lg:col-span-5 rounded-2xl border border-emerald-500/30 bg-emerald-950/20 dark:bg-emerald-950/40 p-4 space-y-3 relative overflow-hidden flex flex-col justify-between">
          ...
          <div className="rounded-2xl rounded-tl-xs bg-emerald-900/40 border border-emerald-500/30 p-3.5 space-y-2 text-xs text-foreground font-sans shadow-lg">
            ...
            <p className="font-semibold text-foreground">
              👋 Hi Strategist! Here is your trajectory update for <strong className="text-emerald-400 font-bold">"{destinationTitle}"</strong>:
            </p>
            ...
            {notifyExpenses && (
              <div className="text-[10px] font-mono text-amber-400/90 flex items-center gap-1 pt-0.5">
                <Bell className="w-3 h-3 text-amber-400 shrink-0" />
                <span>Obligation Check: Next annual commitment in 45 days.</span>
              </div>
            )}

            <div className="text-[9px] text-right font-mono text-emerald-400/80 pt-1 flex items-center justify-end gap-1">
              <span>19:00</span>

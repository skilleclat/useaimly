  ...
  async processUserMessage(
    ...
    if (parsedQuery.isValid && parsedQuery.extractedAmount > 0) {
      ...
      if (sim.affordability.canPhysicallyPay) {
        if (delayDays === 0) {
          responseText = `Based on your account context (${formatCurrency(context.profile.totalLiquidSavings, currency)} liquid reserves and ${formatCurrency(context.profile.monthlyFreeCashFlow, currency)} FCF), you can comfortably afford this ${decisionAmountFormatted} expense. It leaves you with ${remainingLiquid} in liquid reserves (${sim.affordability.obligationsPreservedMonths} months living buffer) and keeps your primary destination "${primaryTitle}" safely on track for ${projectedArrival} without any delay.`;
        } else {
          const extraRequired = formatCurrency(sim.delta.additionalMonthlyAmountRequired || 1875, currency);
          responseText = `Analyzing your account: You have the ${decisionAmountFormatted} in cash today (leaving ${remainingLiquid} in reserves). However, this expense consumes liquid capital that would otherwise fund your "${primaryTitle}" goal (${formatCurrency(context.primaryDestination.currentAmount, currency)} of ${formatCurrency(context.primaryDestination.targetAmount, currency)} saved). Result: It delays your arrival by +${delayDays} days. To neutralize this delay, you can boost your monthly allocation from ${monthlyPace} to +${extraRequired}/month for the next 12 months.`;
        }
      } else {
        responseText = `CRITICAL WARNING: This ${decisionAmountFormatted} expense exceeds your available liquid savings of ${formatCurrency(context.profile.totalLiquidSavings, currency)}. Executing this now would cause an immediate deficit of ${formatCurrency(parsedQuery.extractedAmount - context.profile.totalLiquidSavings, currency)} and jeopardizes your essential living buffer (${context.profile.liquidRunwayMonths} months).`;
      }
